...
def _dump_swe_archive_layout(
    config: MasterConfig, recipe_path: str
) -> None:
    """Decorate exp_NNN/ with the 5/19 archive layout.

    Adds three artifacts on top of what `_log_trajectory_collection` already
    wrote, so the experiment bundle matches the standalone mini_swe_agent
    eval archive shape (e.g.
    `script/dynamo/SWE/results/qwen3-30b-instruct-2507-vllm-v3-*/`):

      * `trajectories/`            — symlink to mini-swe-agent's per-instance
                                     output tree. Linked first because
                                     ``examples.swe_bench.analysis.write_summary``
                                     reads it for per-request token/timing stats.
      * `manifests/`               — copy of the recipe + sibling infra/DGD
                                     YAMLs so the bundle stays self-describing.
      * `summary.txt` +
        `tool_call_timings.jsonl`  — generated by
                                     ``examples.swe_bench.analysis.write_summary``
                                     (which is also runnable standalone for
                                     offline re-aggregation).
    """
    ...
    # from <exp>/manifests/recipe.yaml on a later offline re-run.
    manifests_dir = exp_dir / "manifests"
    manifests_dir.mkdir(exist_ok=True)
    recipe = Path(recipe_path).resolve()
    candidates = [recipe]
    parent = recipe.parent
    for sibling in ("infra.gb300.yaml", "dgd.gb300.yaml"):
        path = parent / sibling
        if path.exists():
            candidates.append(path)
    for src in candidates:
        try:
            shutil.copy(src, manifests_dir / src.name)
        except OSError as exc:
            print(f"[archive] failed to copy {src}: {exc}")
    print(f"[archive] wrote {manifests_dir}/ ({len(candidates)} file(s))")

    # summary.txt + tool_call_timings.jsonl via the shared analysis module.
    # Failure here is non-fatal — the trajectories/ symlink and manifests/
    # are already on disk, and the user can re-run the analysis offline.
    try:
        from examples.swe_bench.analysis import write_summary
        write_summary(exp_dir, model_name)
    except Exception as exc:
        print(f"[archive] write_summary failed: {exc!r}")
...
def main() -> None:
    ...
    try:
        ...
        _dump_swe_archive_layout(config, args.config)
