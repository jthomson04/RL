...
class FusedMoE(PluggableLayer):
    ...
    def _get_hidden_dim(shard_dim: int, ndim: int) -> int:
        ...
        if shard_dim == dim_b:
            return dim_a
        # DEBUG PATCH — capture caller frame + shapes
        import sys, traceback as _tb
        frame = sys._getframe(1)
        _tb.print_stack(frame, limit=5)
        print(f"[FUSED-MOE-DEBUG] _get_hidden_dim shard_dim={shard_dim} ndim={ndim} caller_locals shapes: ", flush=True)
        for k, v in frame.f_locals.items():
            import torch as _t
            if hasattr(v, "shape") and hasattr(v, "ndim"):
                print(f"[FUSED-MOE-DEBUG]   {k}: shape={tuple(v.shape)} ndim={v.ndim}", flush=True)
            else:
                repr_v = repr(v)[:80]
                print(f"[FUSED-MOE-DEBUG]   {k}: {repr_v}", flush=True)
        raise ValueError(
